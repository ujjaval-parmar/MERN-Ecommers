...
const HorizontalCardProduct = ({ category }) => {
    ...
    return (
        <div className='container  mx-auto px-4 my-6   '>
            ...
            <h2 className='font-semibold text-2xl py-4'>Top's <span className='capitalize'>{category}</span></h2>
            ...
            <Swiper
                ...
                {loading ?
                    (
                        ...
                        loadingNumber.map((item, index) => {
                            return (
                                <SwiperSlide
                                    key={index}

                                    className="w-full group bg-white rounded-sm shadow flex  ">

                                    <div className='bg-slate-200  p-4 max-w-[120px] md:max-w-[145px]  w-[40%] animate-pulse'>

                                    </div>


                                    <div className='p-4  w-[60%] overflow-hidden'>
                                        ...
                                        <div className='w-full text-center'>
                                            <button className='
                                             text-white px-3 py-1 rounded-full w-[90%] bg-slate-200 animate-pulse'></button>
                                        </div>
                        ...
                        categoryProducts.length > 0 && categoryProducts.map(product => {
                            return (
                                <SwiperSlide
                                    ...
                                    <NavLink to={`/product-detail/${product._id}`} className='w-full group bg-white rounded-sm shadow flex  '>

                                        <div className='bg-slate-200  p-4 max-w-[120px] md:max-w-[145px]  w-[40%]'>
                                            <img
                                                src={product?.productImage[0]}
                                                alt={product?.productName}
                                                className=' w-full h-full object-scale-down group-hover:scale-110 mix-blend-multiply'
                                            />
                                        </div>
